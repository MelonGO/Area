    ...
    public void run(){
        while(flag){
            ...
            if(father.bFall){//判断球是否已经移出挡板
                ...
                //判断小球是否到达最高点
                if(father.startVY < 0 && Math.abs(father.v_y) <= StartView.UP_ZERO){
                ...
                //判断小球是否撞地
                if(father.y + father.r*2 >= StartView.GROUND_LING && father.v_y >0){//判断撞地条件
                    //改变水平方向的速度
                    father.v_x = father.v_x * (1-father.impactFactor);	//衰减水平方向上的速度
                    //改变竖直方向的速度
                    father.v_y = 0 - father.v_y * (1-father.impactFactor);		//衰减竖直方向上的速度并改变方向
                    if(Math.abs(father.v_y) < StartView.DOWN_ZERO){	//判断撞地后的速度，太小就停止
                        this.flag = false;
                    }
            ...
            else if(father.x + father.r/2 >= StartView.WOOD_EDGE){//判断球是否移出了挡板
